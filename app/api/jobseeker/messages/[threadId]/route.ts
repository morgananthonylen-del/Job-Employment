...
export async function GET(
  ...
  try {
    ...
    // Verify the thread belongs to this job seeker
...
export async function POST(
  ...
  try {
    ...
    const { data: thread, error: threadError } = await supabase
      .from("job_post_threads")
      .select("id, job_seeker_id, job_id")
      .eq("id", threadId)
      .eq("job_seeker_id", authUser.userId)
      .single();
    ...
    // Create the message
    // Ensure sender_type matches the CHECK constraint exactly
    const senderType: "jobseeker" | "business" = "jobseeker";
    ...
    // Update thread's last_message_at to ensure business sees the conversation
