...
function toast({ duration, ...props }: Toast & { duration?: number }) {
  ...
  if (autoDismissDuration === undefined) {
    // Default: 5 seconds for normal, 10 seconds for warning, 5 seconds for destructive
    if (props.variant === "warning") {
      autoDismissDuration = 10000 // 10 seconds
    } else {
...
function useToast() {
  ...
  }, []) // Fixed: removed [state] dependency to prevent infinite loops
